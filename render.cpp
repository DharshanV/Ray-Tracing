...
bool validArgs(uint& w, uint& h, uint& splitCount, const char*& fileName,
               int argc, char* argv[]);
...
int main(int argc, char* argv[]) {
  ...
  Material purpleMarble(vec3f(0.6, 0.3f, 0.1f), vec3f(0.686, 0.329, 0.890),
                        200);
  ...
  renderer.addObject(
      new Plane(vec3f(0, -1, -5), vec3f(0, 1, 0), 20, 20, planeMat));
...
bool validArgs(uint& w, uint& h, uint& splitCount, const char*& fileName,
  ...
  switch (argc) {
    ...
    default:
      ...
      std::cout << "          <width> <height> <splitCount> <fileName>"
                << std::endl;
      return false;
  }
}