...
class Plane : public Object {
   public:
    Plane(const vec3f& center, const vec3f& N, 
        float width, float height, const Material& mat) : p0(center), N(N),
         width(width), height(height), material(mat) { }
    Plane(const vec3f& center, const vec3f& N, 
        float width, float height) : p0(center), N(N),
