...
class Material {
   public:
    ...
    Material(const Material& m) : albedoColor(m.albedoColor), 
            diffuseColor(m.diffuseColor), specularExpo(m.specularExpo) {}
    ...
    void setAlbedo(const vec3f& albedoColor) {
        this->albedoColor = albedoColor;
    }
    void setSpecular(const float& specular) {
        this->specularExpo = specular;
    }
